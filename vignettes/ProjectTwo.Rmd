...
phenology <- read.csv("/cloud/project/data/Phenology.csv")
```
...
```{r, eval = FALSE}
# 
ggplot(data = phenology, aes(x = AnnualTemp, y = Year)) + 
+ geom_line(color="red") +
+ geom_point()
...
```{r}
# joined_data <- merge(butterfly, phenology)
ggplot(joined_data, aes(x=AnnualTemp, y=Day)) +
+ geom_line() +
+ facet_wrap(facets = vars(ButterflySpecies))
```
